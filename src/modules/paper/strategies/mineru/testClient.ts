    ...
    return new Promise((resolve) => {
      ...
      xhr.onload = () => {
        ...
        if (xhr.status === 401 || xhr.status === 403) {
          resolve({ success: false, message: "API Token 无效或已过期，请检查 Token 是否正确" });
          return;
        }
        try {
          ...
          if (data.code === 0 && data.data?.task_id) {
            ...
            resolve({ success: false, message: `连接失败：${data.msg || "未知错误"}` });
          }
        } catch {
      ...
      xhr.onerror = () => resolve({ success: false, message: "网络请求错误，请检查网络设置" });
