...
- **输入框固定底部** — 输入区域使用 `position: sticky` 固定在面板底部，滚动查看历史消息时输入框始终可见
- **输入框自动高度** — textarea 随输入内容自动增长，最大高度 400px，超过后显示滚动条
- **发送按钮优化** — 按钮文字居中显示，新增 `(Enter)` 提示回车键发送
...
- **多模态截图分析** — 启用"框选区域"按钮，支持在 PDF 上拖框截图并附加到对话消息
  - 点击"框选区域"后出现全屏遮罩，鼠标变十字光标
...
- 新增"框选区域"按钮占位（当前灰色不可用，待多模态截图功能实现后启用）
